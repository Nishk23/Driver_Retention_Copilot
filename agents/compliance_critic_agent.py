...
def validate_retention_plan(state: DriverCopilotState) -> CriticVerdict:
    ...
    plan["incentive_options"] = state.get("incentive_options") or {}
    plan["support_tickets"] = state.get("support_tickets") or []
    plan["user_query"] = state.get("user_query") or ""
    plan = normalize_plan_actions(plan)
